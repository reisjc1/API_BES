...
namespace WebApplication1.Controllers
{
    public class LeaseDeskController : ApiController
    {
        ...
        [AcceptVerbs("GET", "POST")]
        [ActionName("GetProcessosVendasFechadosComercial")]
        public IHttpActionResult GetProcessosVendasFechadosComercial(string year, string user)
        {
            List<LD_ContratoModel> listModel = new List<LD_ContratoModel>();
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(bdConnect))
                {
                    ...
                    // 1.  create a command object identifying the stored procedure
                    SqlCommand cmd = new SqlCommand("get_LeaseDesk_ALL_New_Fechados_COMERCIAL", conn);
                    cmd.Parameters.Add("@year1", SqlDbType.NVarChar, 20).Value = year;
        ...
        public IHttpActionResult GetProposta(int? proposalID)
        {
            ...
            try
            {
                var proposalRootObj = proposalBll.GetProposta(proposalID);
